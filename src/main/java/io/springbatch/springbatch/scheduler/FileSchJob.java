...
public class FileSchJob extends QuartzJobBean
{
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException
    {
        ...
        JobParameters jobParameters = new JobParametersBuilder().addLong("id", new Date().getTime()).addString("requestDate", requestDate).toJobParameters();
        ...
        if( jobInstances.size() > 0 )
        {
            for( JobInstance jobInstance : jobInstances)
            {
                ...
                List<JobExecution> jobExecutionList = jobExecutions.stream().filter
                (
                        jobExecution ->
